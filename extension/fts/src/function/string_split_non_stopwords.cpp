...
struct StringSplitNonStopwords {
    static void operation(const ku_string_t& strToSplit, const ku_string_t& separator,
        ...
        auto total = 0u, idx = 0u;
        auto selVector = std::vector(splitStrVec.size(), false);
        for (auto i = 0u; i < splitStrVec.size(); i++) {
            if (splitStrVec[i] == "scarring") {
                printf("hitting this document 60");
            }
            if (stopWordsSet.contains(splitStrVec[i])) {
                continue;
            }
            selVector[i] = true;
            total++;
        }
        result = ListVector::addList(&resultVector, total);
        for (auto i = 0u; i < splitStrVec.size(); i++) {
            if (selVector[i]) {
                ListVector::getDataVector(&resultVector)->setValue(result.offset + idx++, splitStrVec[i]);
            }
        }
    }
};
...
function_set StringSplitNonStopWordsFunction::getFunctionSet() {
    stopWordsSet = {"a", "a's", "able", "about", "above", "according",
